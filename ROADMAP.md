...
**Definition of Done**: `kubectl apply -f machine.yaml` 한 뒤 `kubectl annotate machine/node-a01 onp.io/wake-now=true` 하면 노드가 깨어나고, `Machine.status.state` 가 `Off → Booting → Ready` 로 자동 전이된다. pending pod 감지는 아직 없음 — 트리거는 어노테이션.

**왜 이걸?** — controller / agent / CRD / power provider / 상태 머신을 한 번에 통합. 자동화는 다음 milestone 의 일.

- [ ] kubebuilder v4 초기화 (`kubebuilder init --domain onp.io --repo github.com/<owner>/onp`)
- [ ] `api/v1alpha1/machine_types.go` — 최소 필드 (`nodeName`, `capacity`, `power.provider`, `power.wol.{macAddress, broadcastAddress}`, `status.state`, `status.conditions`)
- [ ] `internal/power/provider.go` — `PowerProvider` 인터페이스 + `Capabilities` + registry
- [ ] `internal/power/wol/provider.go` — wol-agent gRPC 호출, capabilities `{CanPowerOn: true}`
- [ ] `cmd/onp-wol-agent/` — gRPC 서버 (`WakeRequest(mac, broadcastAddr)`)
- [ ] `cmd/onp-controller/` + Machine reconciler:
  - 어노테이션 `onp.io/wake-now=true` 감지 → PowerOn (provider 호출) → `state=Booting`
  - Node watch → Ready 신호 시 `state=Ready` + 어노테이션 제거
  - `bootTimeout` (기본 10m) 경과 시 `state=Failed`
- [ ] 매니페스트: controller Deployment, wol-agent DaemonSet (`hostNetwork: true`, 컨트롤 플레인 nodeSelector)
- [ ] **검증**: kind 클러스터에 배포 → 진짜 노드 Machine 등록 → 어노테이션으로 깨움, 상태 전이 확인
...
- [ ] `charts/onp/` Helm chart (controller + wol-agent + shutdown-agent + CRD + RBAC + PSA)
