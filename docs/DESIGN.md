...
- **`onp-wol-agent`** — DaemonSet, `hostNetwork: true`. **항상 켜져 있는 노드(=컨트롤 플레인 또는 그에 준하는 노드)에만** 배치한다 (nodeSelector로 제한). 컨트롤러로부터 HTTP/JSON으로 "이 MAC 주소로 매직 패킷 보내" 요청을 받아, 호스트 네트워크 인터페이스에서 L2 브로드캐스트로 송신한다. 상태를 가지지 않는다 — 받은 요청을 그대로 패킷으로 변환할 뿐. (단일 one-shot 요청이라 gRPC 대신 표준 라이브러리 HTTP로 충분하다 — protoc 툴체인과 추가 의존성을 피한다. 전송은 `PowerProvider` 구현 내부에 숨겨져 추후 교체 가능.)
...
flowchart LR
    ...
    Pod -.->|watch| Ctrl
    Ctrl -->|"HTTP:<br/>WakeRequest(mac)"| WoL
...
sequenceDiagram
    ...
    Ctrl->>WoL: HTTP WakeRequest(mac, broadcastAddr)
